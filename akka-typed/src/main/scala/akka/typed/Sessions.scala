...
object Sessions {
  ...
  trait Process[+T] { this: ProcessImpl[T] =>
     ...
     */
    def filter(pred: T => Boolean): Process[T] = flatMap(t => if (pred(t)) process(t) else fail)
     ...
     */
    def collect[U](pf: PartialFunction[T, U]): Process[U] = flatMap(t => if (pf.isDefinedAt(t)) process(pf(t)) else fail)
   ...
   */
  def process[T](value: T): Process[T] = Value(value)
...
object SessionExample extends App {
    ...
    for {
      ...
      cmd match {
        case GetIt(which, replyTo) =>
           ...
           */
          fork(spinOff race timeout(5.seconds)).then(run(server, backend))
      }
    }
  ...
  private def registerBackend: Process[Channel[BackendCommand]] = {
    ...
    read(registered).then(process(service))
  }
  ...
  private def runBackend(service: Channel[BackendCommand]): Process[Nothing] =
    for {
      ...
      case GetThingCode(auth, replyTo) =>
        ...
        fork(read(code).map { c => c.replyTo ! TheThing(c.which); halt }).then(runBackend(service))
    }
