      ...
      _state match {
        ...
        case NeedsInternalInputs =>
          ...
          stack(ptr - 3) match {
            ...
            case Race(_, _) =>
              other.asInstanceOf[Traversal].cancel()
              pop() // discard right
              pop() // discard left
    ...
    def dispatchTrampoline(): Unit = {
      ...
      pop() match {
        case s: TerminationImpl.Sentinel =>
          ...
          while (ptr > 0)
            pop() match {
              case Trap(_, cleanup) => try cleanup(s.back.signal) catch { case NonFatal(_) => }
...
private[typed] sealed abstract class TerminationImpl(val signal: Termination) extends ProcessImpl[Nothing] {
  ...
  val value = new TerminationImpl.Sentinel(this)
}
private[typed] object TerminationImpl {
  class Sentinel(val back: TerminationImpl)
}
